# GNN Topology-Aware Risk Scoring - Quick Start

## Overview
Novel AI feature using Graph Neural Networks for topology-aware risk scoring of cloud infrastructure.
The GNN outputs a per-graph risk probability and per-node attention weights; concrete
attack-path enumeration is performed by the separate `attack_path_analyzer` module.
...
- Learns risk patterns over infrastructure graphs (e.g. Internet → Public Instance → Database)
- Credential-leak topologies: Public S3 → Credentials → Resources
- Privilege-escalation topologies: Exposed admin panels → IAM roles → Data

> **Note:** The GNN produces a risk *probability* per graph plus node-level attention.
> Concrete multi-hop attack path chains are generated by `attack_path_analyzer.py`.
...
- "Graph Neural Networks for Topology-Aware Infrastructure Risk Scoring"
